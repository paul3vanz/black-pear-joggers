...
export namespace rankingsSelectors {
  ...
  export const getRankingsByYear = createSelector(
    selectAllRecords,
    (allRecords: Ranking[], year: number) => {
    ...
    return allRecords
      ...
      .reduce(
        (accumulator, currentValue) => {
          ...
          const currentYearIndex = accumulator.findIndex(
          ...
          if (currentYearIndex === -1) {
            ...
            if (
              ...
              accumulator[currentYearIndex].highest.ranking
            ) {
              accumulator[currentYearIndex].highest = currentValue;
            }
            if (
              currentValue.ranking <
              accumulator[currentYearIndex].lowest.ranking
            ) {
              accumulator[currentYearIndex].lowest = currentValue;
            }
            if (
              new Date(currentValue.date) >
              new Date(accumulator[currentYearIndex].last.date)
            ) {
              accumulator[currentYearIndex].last = currentValue;
            }
            if (
              new Date(currentValue.date) <
              new Date(accumulator[currentYearIndex].first.date)
            ) {
      ...
      .map(rankingStat => {
        const improvement =
          rankingStat.last.ranking - rankingStat.first.ranking;
